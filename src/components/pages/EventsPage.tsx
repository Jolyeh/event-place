      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="input input-bordered w-full pr-10 bg-base-200 border-primary/15 focus:border-primary/50 focus:outline-none text-sm rounded-xl h-11 placeholder:text-base-content/25"
            />
            {search && (
          ...
          <button
            ...
            className={`btn btn-sm rounded-xl gap-2 text-xs uppercase tracking-wider h-11 px-5 border transition-all duration-200 ${filtersOpen || hasFilters
          ...
          <div className="relative">
            <select
              ...
              className="select select-bordered select-sm w-full bg-base-200 border-primary/15 focus:border-primary/50 focus:outline-none text-sm rounded-xl h-11 min-h-0 appearance-none"
            >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {categories.map(cat => (
                      <button
                        ...
                        className={`badge py-3 px-3 text-[11px] gap-1.5 cursor-pointer transition-all duration-200 ${activeCat === cat.name
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CITIES.map(city => (
                        <button
                          ...
                          className={`badge py-2.5 px-3 text-[11px] cursor-pointer transition-all duration-200 ${activeCity === city
                    ...
                    <div className="flex flex-wrap gap-2">
                      {PRICE_RANGES.map((pr, i) => (
                        <button
                          ...
                          className={`badge py-2.5 px-3 text-[11px] cursor-pointer transition-all duration-200 ${priceRange === i
                            ? "badge-primary border-primary"
                            : "badge-outline border-primary/12 text-base-content/45 hover:border-primary/35"
                            }`}
                        >
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-6 scrollbar-hide">
          {categories.map(cat => (
            <button
              key={cat.name}
              onClick={() => setActiveCat(cat.name)}
              className={`shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-medium transition-all duration-200 border ${activeCat === cat.name
        ...
        <AnimatePresence mode="wait">
          {filtered.length === 0 ? (
            ...
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filtered.map((ev, i) => (
                <motion.div
                  key={ev.id}
